function setupListeners() {
  ...
  const languagePickerDropdown = document.getElementById(
    "languagePickerDropdown"
  );

  if (languagePicker instanceof HTMLButtonElement) {
    languagePicker.addEventListener("click", () => {
      if (languagePickerDropdown instanceof HTMLDivElement) {
        languagePickerDropdown.classList.toggle("flex");
        languagePickerDropdown.classList.toggle("hidden");
      }
    });
  }
}
